...
combinations = [('lr', 'movement', 'LR-MM')]
...
for combo in combinations:
    ...
    for x in r:
        ...
        fname = 'sub{sub}_{stim}_{type}_lfreq_short'.format(sub=x, stim=combo[0], type=combo[1])
        ...
        test = ClassifierTester(subj_range=[x, x+1], data_source='live-movement', stim_select=combo[0], stim_type=combo[1],
                                p_select='gridsearch', p_select_frac=1, result_metrics=['acc', 'f1', 'rec', 'prec', 'roc'],
                                p_n_jobs=-1, tmin=0, tmax=4, f_name=fname, f_path=fpath, random_state=rand,
                                live_layout='m_cortex', filter_bounds=(2., 15.))
        ...
        test = ClassifierTester(subj_range=[x, x + 1], data_source='live-movement', stim_select=combo[0],
                                ...
                                result_metrics=['acc', 'f1', 'rec', 'prec', 'roc'], p_n_jobs=-1, tmin=0, tmax=2,
