...
for combo in combinations:
    ...
    fname = 'loo_{stim}_{type}_openbci_'.format(stim=combo[0], type=combo[1])
    ...
    test.run_batch_test(batch_size=5, n_times=5, sk_test=True, test_split=0.2, cross_val_times=5, nn_test=False,
                        split_subject=True)
    del test
    ...
    test = ClassifierTester(subj_range=[1, 6], data_source='live-movement', stim_select=combo[0], stim_type=combo[1],
                            result_metrics=['acc', 'f1', 'rec', 'prec', 'roc'], tmin=-1, tmax=4, notch=None,
                            f_name=fb_name, f_path=ml_path, random_state=rand, p_select=None, p_select_frac=1,
